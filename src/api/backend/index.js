...
export const signin = (account) => {
  ...
  return http.post('/security/signin/signature', body, { headers: { Authorization: `Signature ${ signature }` } })
  // return fetch(`${ API_URL }/security/signin/signature`, {
  //   method: 'POST',
  //   headers: { Authorization: `Signature ${ signature }`, ...COMMON_REQUEST_CONFIG.headers },
  //   body: { purpose: 'laborx-session' }
  // })
}

export const uploadImage = (file: FileModel) => fetch(`${ API_URL }/media/image/upload`, {
  method: 'POST',
  body: file,
